...
boost::tuple<Type, Type> SDSE(const std::vector<Type>& price, const Type& r, const Type& T)
{
	...
	int M = price.size();
	cout << M << "simulations" << endl;
	Type sd = sqrt((temp1 - (1 / M) * temp2 * temp2 * exp(-2 * r * T)) / (double)((double)M - 1.0));
	Type se = sd / sqrt(M);
...
int main()
{
	...
	for (int i = 0; i < vecBatch.size(); i++)
	{
		...
		//Group D. a.)
		// Print SD and SE.
		boost::tuple<double, double> tupleCall = SDSE<double>(vecCallPrice, myOption.r, myOption.T);// to calculate the SD and the SE for the given put call option prices
