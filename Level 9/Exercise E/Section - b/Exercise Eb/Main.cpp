...
void main()
{
	...
	for (int i = 0; i < vecBatch.size(); i++)
	{
		...
		vector<double> vecC(n, 0.0), vecP(n, 0.0);
		for (int j = 0; j < vecU.size(); j++)
		{
			...
			option1.toggle();
			cout << "Underlying stock S=" << vecU[j] << " Call price = " << vecC[j] << " Put Price = " << vecP[j] << endl;
		}
	...
	try
	{
		printOneExcel(vecU,functionResult[0],string("Call price against underlying put price"));
		printOneExcel(vecU, functionResult[1], string("Put price against underlying stock price"));
	}
